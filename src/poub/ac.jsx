...
const repartirZones = (geomParcelle, zonesFeatures, surfaceTotale) => {
  ...
  const N = 60; // 3600 points d'échantillonnage
  const compte = {};
  let dedansTotal = 0;
  const zonesAnn = zonesFeatures.map((z) => ({ lib: z.libelle, ann: anneauxDe(z.geometry) }));
  for (let i = 0; i < N; i++) for (let j = 0; j < N; j++) {
...
const chercherKillers = (d, jeu) => {
  ...
  /* Mixité sociale publiée par le PLU : on extrait seuil et taux si présents. */
  const sps = d.prescriptions.find((p) => /mixit[ée].*social|programme de logements/i.test(libP(p)));
  if (sps) {
    const m = libP(sps).match(/(\d{3,5})\s*\/\s*(\d{1,2})/);
    if (m) {
      d.sps_detecte = { seuil: +m[1], taux: +m[2] / 100 };
      push('info', 'SPS', `Mixité sociale imposée : ${m[2]} % de logement social au-delà de ${num(+m[1])} m² de SDP. Appliquée automatiquement au bilan si le seuil est franchi.`, '');
    } else {
  ...
  if (geomParam) {
    ...
    try {
      ...
      if (zs.length) {
        ...
        d.zones_features = feats.map((f) => ({ libelle: f.properties.libelle, geometry: f.geometry }));
        if (d.zonages_multiples.length > 1) {
          ...
          d.repartition = repartirZones(d.geometrie, d.zones_features, surfT);
          ...
          <div className="pan" style={{ padding: 24 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 10, marginBottom: 18 }}>
              {[
                ...
                mc ? ['Charge foncière (méd.)', eur(mc.median)] : null,

...
const dansPolygone = (lon, lat, anneaux) => {
  // pair-impair sur tous les anneaux (gère les trous)
  let ok = false;
...
const repartirZones = (geomParcelle, zonesFeatures, surfaceTotale) => {
  const annP = anneauxDe(geomParcelle);
  if (!annP.length || !zonesFeatures?.length) return null;
  // bbox de la parcelle
  const pts = annP.flat();
  const lons = pts.map((p) => p[0]), lats = pts.map((p) => p[1]);
  const l0 = Math.min(...lons), l1 = Math.max(...lons);
  const t0 = Math.min(...lats), t1 = Math.max(...lats);
  const N = 40; // 1600 points — suffisant pour ±2 % de précision
  const compte = {};
  let dedansTotal = 0;
  // Pré-calcul : anneaux + bbox de chaque zone pour rejet rapide
  const zonesAnn = zonesFeatures.map((z) => {
    const ann = anneauxDe(z.geometry);
    const zp = ann.flat();
    return {
      lib: z.libelle, ann,
  ...
  for (let i = 0; i < N; i++) for (let j = 0; j < N; j++) {
    ...
    if (!dansPolygone(lon, lat, annP)) continue;
    dedansTotal++;
    for (const z of zonesAnn) {
      if (lon < z.bx0 || lon > z.bx1 || lat < z.by0 || lat > z.by1) continue; // rejet bbox
      if (z.ann.length && dansPolygone(lon, lat, z.ann)) { compte[z.lib] = (compte[z.lib] || 0) + 1; break; }
    }
  }
...
const bilan = (env, d, jeu, tirage = null) => {
  ...
  // sinon le DVF réel du terrain SI fiable, sinon l'hypothèse.
  const dvfOk = d.prix_m2_dvf?.fiable === true;
  const pv = tirage ? g('prix_vente_m2') : (dvfOk ? d.prix_m2_dvf.median : g('prix_vente_m2'));
...
const scenarios = (env, d, jeu, mc) => {
  if (!env || env.sdp <= 0) return null;
  const dvfOk = d.prix_m2_dvf?.fiable === true;
  const pv = dvfOk ? d.prix_m2_dvf.median : jeu.prix_vente_m2.v;
  const shab = env.shab;
...
const monteCarlo = (d, regles, jeu, N = 1500) => {
  ...
  for (let i = 0; i < N; i++) {
    ...
    // Prix de vente : si DVF réel FIABLE, on tire dans SA distribution (P25–médiane–P75)
    if (d.prix_m2_dvf?.fiable === true) {
  ...
  if (d.code_insee && d.section_dvf) {
    ...
    for (const v of [...new Set(variantes)]) {
      try {
        ...
        // L'API Etalab renvoie la clé "donnees" (source: DVF-app/app.py)
        const arr = dvf.donnees || dvf.mutations || null;
        if (arr && arr.length) { muts = arr; break; }
    ...
    if (muts) {
      ...
      if (comparables.length >= 5) {
        ...
        d.prix_m2_dvf = {
          ...
          n: comparables.length, echelle: 'section',
          fiable: plausible,
        ...
        d.prix_m2_dvf = { median: moy, p25: Math.round(moy * 0.85), p75: Math.round(moy * 1.15), n: comparables.length, echelle: 'section', fiable: false, note: `Seulement ${comparables.length} vente(s) comparables — indicatif, non utilisé dans le bilan.` };
          ...
          {mc ? (
            <>
              ...
              {dossier.comparables?.length > 0 && (
                <div className="pan" style={{ padding: 18, marginBottom: 16 }}>
                  <div className="row" style={{ justifyContent: 'space-between', marginBottom: 12 }}>
                    <div className="eyebrow">Vendu dans le voisinage immédiat</div>
                    <div className="row" style={{ gap: 8 }}>
                      {dossier.prix_m2_dvf && (
                  ...
                  <div style={{ fontSize: 10.5, color: C.txt3, marginTop: 8, lineHeight: 1.5 }}>
                    Ventes enregistrées (DVF) dans la section cadastrale, dédupliquées par mutation, ventes en bloc écartées.
                    Le DVF ne renseigne ni l'état du bien ni son contexte : ces prix situent le marché, ils ne valorisent pas un bien précis.
                  </div>
                </div>
              )}
              ...
              {bil.coherence === 'haute' && (
                <div className="pan" style={{ padding: 14, marginBottom: 14, borderColor: C.ambre2 }}>
                  <div style={{ fontSize: 12.5, color: C.txt2, lineHeight: 1.5 }}>
                    ...
                    la charge foncière représente {pct(bil.ratio_cf_ca)} du CA, au-dessus de la fourchette usuelle en promotion (10–25 %).
